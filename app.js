...
// City to timezone mapping - USA & Australia
const cityTimezones = {
    // USA - Pacific Time
    ...
    // AUSTRALIA - Australian Eastern Time (AEDT/AEST)
    ...
    'Mornington Peninsula': 'Australia/Melbourne',
    'Pakenham': 'Australia/Melbourne',
    'Colac': 'Australia/Melbourne',
    'Tasmania': 'Australia/Hobart',

    // AUSTRALIA - Australian Central Time (ACDT/ACST)
    'Adelaide': 'Australia/Adelaide',
    'Darwin': 'Australia/Darwin',

    // AUSTRALIA - Australian Western Time (AWST)
    'Perth': 'Australia/Perth',
    'Fremantle': 'Australia/Perth',
    'Mandurah': 'Australia/Perth',
    'Stirling': 'Australia/Perth',
    'Cockburn': 'Australia/Perth',
    'Midwest WA': 'Australia/Perth',

    // AUSTRALIA - Other major cities
    ...
    try {
        ...
        fs.createReadStream(req.file.path)
            ...
            .on('data', (row) => {
                ...
                const findCol = (names) => {
                    const found = Object.keys(row).find(k => names.includes(k.toLowerCase().trim()));
                    return found ? row[found] : null;
                };

                results.push({
                    businessName: findCol(['business name', 'business']),
                    email: findCol(['email address', 'email']),
                    websiteUrl: findCol(['website url', 'website', 'url']),
                    whatStoodOut: findCol(['what stood out', 'stood out']),
                    city: findCol(['city', 'location', 'town']),
                    status: findCol(['status']) || 'pending',
                    sentCount: 0,
                    lastSent: null
                });
            })
