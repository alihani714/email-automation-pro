...
function isWithinSendingWindow() {
    // TEMPORARY: Disabled for testing - allows sending at any time
    return true;

    /* Original time window check (9 AM - 12:30 PM):
    const now = new Date();
...
async function processQueue() {
    ...
    console.log(`[Campaign] Processing queue - ${pendingClients.length} pending clients`);
    ...
    if (!isWithinSendingWindow() || !checkDailyLimit()) {
        console.log('[Campaign] Outside sending window or daily limit reached. Waiting 5 minutes...');
        campaignState.processTimeout = setTimeout(processQueue, 5 * 60 * 1000);
        return;
    }

    const nextClient = pendingClients[0];
    console.log(`[Campaign] Sending email to: ${nextClient.email} (${nextClient.businessName})`);
