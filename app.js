...
sgMail.setApiKey('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');
...
function loadState() {
  try {
    if (fs.existsSync('campaign-state.json')) {
      ...
      campaignState.processTimeout = null; // Reset timeout
    }
  } catch (error) {
...
app.post('/api/settings', (req, res) => {
  Object.assign(campaignState.settings, req.body);
  saveState();
...
app.get('/api/log', (req, res) => {
  res.json(campaignState.sentLog);
});
...
<body>
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                <h3>TODAY'S COUNT</h3>
                <div class="value" id="dailyCount">0 / 100</div>
            </div>
        </div>

        <div class="controls">
            <button class="btn-start" onclick="startCampaign()">▶️ Start Campaign</button>
            <button class="btn-stop" onclick="stopCampaign()">⏸️ Stop Campaign</button>
                    ...
                    <tbody id="clientsBody">
                        <tr>
                            <td colspan="5" style="text-align: center; padding: 2rem; color: #6c757d;">
                                No clients loaded. Upload a CSV file to get started.
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <script>
        async function fetchStatus() {
            ...
            document.getElementById('dailyCount').textContent = data.dailyCount + ' / ' + data.dailyLimit;
            
            const statusDiv = document.getElementById('campaignStatus');
            statusDiv.className = 'campaign-status ' + (data.active ? 'campaign-active' : 'campaign-inactive');
            statusDiv.textContent = data.active ? '🟢 Campaign Active' : '🔴 Campaign Stopped';
        }
        ...
        async function fetchClients() {
            ...
            if (clients.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2rem; color: #6c757d;">No clients loaded.</td></tr>';
                return;
            }
        ...
        async function startCampaign() {
            ...
            const data = await res.json();
            alert(data.success ? '✅ Campaign started!' : '❌ ' + data.error);
            fetchStatus();
        }
        ...
        async function stopCampaign() {
            ...
            const data = await res.json();
            alert(data.success ? '⏸️ Campaign stopped!' : '❌ Error');
            fetchStatus();
        }
        ...
        async function uploadCSV(input) {
            ...
            const data = await res.json();
            if (data.success) {
                alert(\`✅ Uploaded \${data.count} clients!\`);
                fetchStatus();
                fetchClients();
            } else {
...
app.listen(PORT, () => {
  console.log('\n╔══════════════════════════════════════════════════════╗');
  console.log('║     📧 EMAIL AUTOMATION PRO - WEB APPLICATION       ║');
  console.log('╚══════════════════════════════════════════════════════╝\n');
